...
TTS_Init(void);
extern void
TTS_Speak(u8 u8CodeType, u8 *pTxt, u8 u8TxtLen, u8 u8RepeatInterval, u8 u8ReadTimes);
extern void
TTS_EnableSound(void);							//允许语音输出
extern void
TTS_DisableSound(void);							//禁止语音输出

/**内部接口 **************************************************************************************/
void
TTS_Process(void);
void
TTS_ReadMsgProcess(void);
void
TTS_SendByte(u8 u8Data);
void
TTS_Stop(void);
void
TTS_Sleep(void);
...
TTS_Wakeup(void);		//唤醒
BOOL
TTS_IsChipIdle(void);	//语音播报状态
void
TTS_SendFrame(u8 *pFrame, u8 u8FrameLen);
void
TIM_Tts_Configuration();

/**ISR *******************************************************************************************/
extern void
Isr_TTS_Timeout10ms(void);
extern void
Isr_TTS_Timeout1s(void);	//延时关闭TTS音源
extern void
T1_Voice_Interrupt(void);
