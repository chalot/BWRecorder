...
void CAN_ReceiveFrame(CANRAWFRAME *pFrame) {
	...
	/*填充帧附加信息*/
	pFrame->dir = 1; /*接收方向*/
	pFrame->tick = (time.minute * 60 + time.second) * 1000
			+ (GetCurTicks() * 10); /*当前时刻*/
}
...
void TRACE_CANRawFrame(CANRAWFRAME *pFrame) {
	TRACE_(CAN_STAT, NULL,
			"[RAWFRAMEINFO] %d, %x, %s, %d, %x %x %x %x %x %x %x %x ",
			pFrame->tick, pFrame->id, (pFrame->dir == 0) ? "Tx" : "Rx",
			pFrame->len, pFrame->data[0], pFrame->data[1], pFrame->data[2],
			pFrame->data[3], pFrame->data[4], pFrame->data[5], pFrame->data[6],
			pFrame->data[7]);
}
