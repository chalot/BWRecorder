	...
	switch (tIRFSM.u8ReceivedStatus)
	{
		...
		case _IR_RECEIVEDSTATUS_CONTINUE:
			  ...
			  &&(tIRFSM.u32CodeTime <=_IR_CODE_TIME_SEQUENTIAL + _IR_CODE_TIME_BIAS) )
			{
				...
				//不是第一个连发码,遥控器支持音量连发
				else if ((_IR_CMD_VOLUME_PLUS == tIRFSM.u8RecievedCode) || (_IR_CMD_VOLUME_MINUS == tIRFSM.u8RecievedCode))
				{
					//红外接收缓冲区是否已满？
					if (tIRFSM.u8Available_Rx < _BUFFER_LENGTH_IR_Rx_)
					{
...
void	Isr_IR_Timeout100us(void)
{
	...
	if(tIRFSM.fCodeTimeEn)
	{
		...
		//接收超时
		if (((tIRFSM.u8ReceivedStatus <= _IR_RECEIVEDSTATUS_32BITSDATA) && (tIRFSM.u32CodeTime >= _IR_DATA_LIMITATION_TIME))
		  ||((tIRFSM.u8ReceivedStatus == _IR_RECEIVEDSTATUS_INTERVAL) && (tIRFSM.u32CodeTime >= _IR_INTERVAL_LIMITATION_TIME)))
		{
			//码型时长禁止
			tIRFSM.fCodeTimeEn = 0;
