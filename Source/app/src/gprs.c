...
"+CREG: ",
	...
	/*FinalResultCode*/
	"CONNECT", "+CRING:", "NO CARRIER", "ERROR", "NO DIALTONE", "BUSY", "NO ANSWER", "+CME ERROR:", "+CMS ERROR:",
...
static u8 PhoneNo_ConvertBaseToAscii(u8 *pcNo, _GPRS_SENDPDUSMSType *ptBaseNo, u8 nInternational);
static u8 GPRS_StoreSMSContent(u8 u8SMS_DCS, u16 u16Code, s16 s16Index, s16 s16Bytes, s16 s16SequenceLongSMS);
...
TRACE_(QS_USER, NULL, "[initialing], step = %d, cmd = %s", initStep, ATCMD[aInitSteps[initStep]]);

		//数据提交到发送串口
GPRS_WriteData((u8*) ATCMD[aInitSteps[initStep]], strlen_(ATCMD[aInitSteps[initStep]]));
}
...
u16 u16RecvData;
#define STRING_MAX	7
static char* ATRESP[] = { "OK", "ERROR", "^SISR", "^SISW", "+CMS ERROR", "+CME ERROR", "NO CARRIER", };
...
TRACE_(QS_USER, NULL, "[connecting], step = %d, cmd = %s ", connectStep, au8Cmd);
...
//提示发送结果
else if (memcmp_(pBuffer, (u8*) "^SISW: 0,1", strlen_("^SISW: 0,1")) && (len == strlen_("^SISW: 0,1"))) {
	//报告数据发送成功
	QACTIVE_POST(AO_Gprs, Q_NEW(QEvt, AT_SENDOK_SIG), NULL);
}
else {
...
//取号码,BCD类型
tSMS_OA.au8Number[i] = SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage));
pu8GPRSMessage += 2;
}
...
pu8GPRSMessage += 2;
				...
				//第6段> 短信服务中心时间戳(SCTS)
u16SMS_Year = (u16) BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage))) + 2000;	//年(BCD)-"50"
pu8GPRSMessage += 2;
u8SMS_Month = BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)));	//月(BCD)-"60"
pu8GPRSMessage += 2;
u8SMS_Day = BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)));	//日(BCD)-"01"
pu8GPRSMessage += 2;
u8SMS_Hour = BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)));	//时(BCD)-"90"
pu8GPRSMessage += 2;
u8SMS_Minute = BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)));	//分(BCD)-"33"
pu8GPRSMessage += 2;
u8SMS_Second = BCD2Decimal(SwapNibble(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)));	//秒(BCD)-"72"
pu8GPRSMessage += 2;
...
while (u8SMS_UDHI_Len) {
	...
	case 0x00:
		//元素A长度
		u8SMS_UDHI_ElementA_Len = GPRS_GetPDUSMS_HexByte(pu8GPRSMessage);
		pu8GPRSMessage += 2;
		...
		if (u8SMS_UDHI_ElementA_Len == 3) {
			...
			pu8GPRSMessage += 2;
			//链接短消息总数
			u8SMS_LongMsg_Total = GPRS_GetPDUSMS_HexByte(pu8GPRSMessage);
			pu8GPRSMessage += 2;
			//链接短消息当前顺序号
			u8SMS_LongMsg_Sequence = GPRS_GetPDUSMS_HexByte(pu8GPRSMessage);
			pu8GPRSMessage += 2;
...
if ((u8SMS_DCS & 0x0C) == 0x00) {
	//7位模式解码
	u32Coded = GPRS_SMS_7BitsDecode(GPRS_GetPDUSMS_HexByte(pu8GPRSMessage), 0, 0);
	pu8GPRSMessage += 2;
	...
	if (u8Byte) {
		//存储短消息内容
		if (!GPRS_StoreSMSContent(u8SMS_DCS, (u16) (u32Coded & 0xFFFF), s32SMS_Store_Index, u8Byte,
				u8SMS_LongMsg_Sequence)) {
			//出错处理
			//		GPRS_SMS_Clear();
			s_u8PDUSMSRx_LenCount = 0;
...
//8位编码?
	...
	//存储短消息内容
	if (!GPRS_StoreSMSContent(u8SMS_DCS, (u16) GPRS_GetPDUSMS_HexByte(pu8GPRSMessage), s32SMS_Store_Index, 1,
			u8SMS_LongMsg_Sequence)) {
		//出错处理
		//		GPRS_SMS_Clear();
...
//16位Unicode编码?
	...
	u32Coded = ((((u16) GPRS_GetPDUSMS_HexByte(pu8GPRSMessage)) << 8) & 0xFF00);
	pu8GPRSMessage += 2;
	u32Coded |= ((u16) GPRS_GetPDUSMS_HexByte(pu8GPRSMessage) & 0x00FF);
	pu8GPRSMessage += 2;

	//存储短消息内容
	if (!GPRS_StoreSMSContent(u8SMS_DCS, u32Coded, s32SMS_Store_Index, 2, u8SMS_LongMsg_Sequence)) {
		//出错处理
		//		GPRS_SMS_Clear();
...
u8DecodedByte = (u8PDUByte << u8GPRS_7bitDecode_Sequence) | u8GPRS_7bitDecode_LeftCode;
...
/*******************************************************************************************
 ...
 设计：	廖颖
 *******************************************************************************************/
u8 PhoneNo_ConvertBaseToAscii(u8 *pcNo, _GPRS_SENDPDUSMSType *ptBaseNo, u8 nInternational) {
u8 nLen = 0;
u8 nSkipDigits = 0;
u8 i = 0;
 ...
 *出    口：	1=存储成功，=0存储失败
 *******************************************************************************************/
u8 GPRS_StoreSMSContent(u8 u8SMS_DCS, u16 u16Code, s16 s16Index, s16 s16Bytes, s16 s16SequenceLongSMS) {
			//是超长短消息?
