...
//GPS响应字符串
char* GPS_ResponseString[_GPS_RESPONSE_NUMBER_] =
{
	"$GPRMC",		//GPS输出00：推荐定位信息
...
//				break;
			...
			TRACE_(QS_USER, NULL, "[GPRMC] %d-%d-%d:%d-%d-%d, Lon=%x%x%x%x, Lat=%x%x%x%x, Speed=%x%x, total_Sat = %d, viewd_Sat = %d",
					tGpsInfo.u16Year, tGpsInfo.u8Month, tGpsInfo.u8Day, tGpsInfo.u8Hour,
					tGpsInfo.u8Minute, tGpsInfo.u8Second,
					tGpsInfo.Longtitude[0], tGpsInfo.Longtitude[1], tGpsInfo.Longtitude[2], tGpsInfo.Longtitude[3],
					tGpsInfo.Latitude[0], tGpsInfo.Latitude[1], tGpsInfo.Latitude[2], tGpsInfo.Latitude[3],
					tGpsInfo.Speed[0], tGpsInfo.Speed[1],tGpsInfo.u8VisibleSatellites, tGpsInfo.u8ValidSatellites
					);
 			break;
...
u16		GPS_GetSegmentResponseCode(u8 u8StartIndex, u8 u8EndIndex)
{
	...
	for(nResCode = u8StartIndex; nResCode <= u8EndIndex; nResCode++)
	{
		...
		//比较响应字符串是否已知的字符串
		if(memcmp_((char*)rpcGPSMessage, GPS_ResponseString[nResCode], nLen))
		{
			//找到响应字符串，代码=ResCode
			break;
		}
...
u16	 GPS_SpeedAverage(u16 u16Speed)
{
	...
	//2点：平均速度 = (速度i + 速度i-1) / 2
	case	2:
		u16AverageSpeed = (u16)((tSpeedSmooth.aSpeedArray[0] + tSpeedSmooth.aSpeedArray[1]) / 2);
		break;

	//3点：平均速度 = 0.3 * 速度i + 0.45 * 速度i-1 + 0.25 * 速度i-2
	case	3:
		u16AverageSpeed = (u16)((tSpeedSmooth.aSpeedArray[0] * 6 + tSpeedSmooth.aSpeedArray[1] * 9 + tSpeedSmooth.aSpeedArray[2] * 5) / 20);
		break;

	//4点：平均速度 = 0.25 * 速度i + 0.4 * 速度i-1 + 0.2 * 速度i-2 + 0.15 * 速度i-3
	case	4:
		u16AverageSpeed = (u16)((tSpeedSmooth.aSpeedArray[0] * 5 + tSpeedSmooth.aSpeedArray[1] * 8 + tSpeedSmooth.aSpeedArray[2] * 4 + tSpeedSmooth.aSpeedArray[3] * 3) / 20);
		break;

	//5点：平均速度 = 0.2 * 速度i + 0.375 * 速度i-1 + 0.175 * 速度i-2 + 0.15 * 速度i-3 + 0.1 * 速度i-4
	case	5:
		u16AverageSpeed = (u16)((tSpeedSmooth.aSpeedArray[0] * 8 + tSpeedSmooth.aSpeedArray[1] * 15 + tSpeedSmooth.aSpeedArray[2] * 7 + tSpeedSmooth.aSpeedArray[3] * 6 + tSpeedSmooth.aSpeedArray[4] * 4) / 40);
		break;
...
void	 GPS_AltitudeAverage(s16 shAltitude)
{
	...
	//2点：平均高度 = (高度i + 高度i-1) / 2
	case	2:
		shAverageAltitude = (s16)((tAltitudeSmooth.aAltitudeArray[0] + tAltitudeSmooth.aAltitudeArray[1]) / 2);
		break;

	//3点：平均高度 = 0.3 * 高度i + 0.45 * 高度i-1 + 0.25 * 高度i-2
	case	3:
		shAverageAltitude = (s16)((tAltitudeSmooth.aAltitudeArray[0] * 6 + tAltitudeSmooth.aAltitudeArray[1] * 9 + tSpeedSmooth.aSpeedArray[2] * 5) / 20);
		break;

	//4点：平均高度 = 0.25 * 高度i + 0.4 * 高度i-1 + 0.2 * 高度i-2 + 0.15 * 高度i-3
	case	4:
		shAverageAltitude = (s16)((tAltitudeSmooth.aAltitudeArray[0] * 5 + tAltitudeSmooth.aAltitudeArray[1] * 8 + tAltitudeSmooth.aAltitudeArray[2] * 4 + tSpeedSmooth.aSpeedArray[3] * 3) / 20);
		break;

	//5点：平均高度 = 0.2 * 高度i + 0.375 * 高度i-1 + 0.175 * 高度i-2 + 0.15 * 高度i-3 + 0.1 * 速度i-4
	case	5:
		shAverageAltitude = (s16)((tAltitudeSmooth.aAltitudeArray[0] * 8 + tAltitudeSmooth.aAltitudeArray[1] * 15 + tSpeedSmooth.aSpeedArray[2] * 7 + tAltitudeSmooth.aAltitudeArray[3] * 6 + tAltitudeSmooth.aAltitudeArray[4] * 4) / 40);
		break;
...
void GPS_SyncSystemTime()
{
	...
	TRACE_(QS_USER, NULL, "[GPS] Sync time: %d-%d-%d:%d-%d-%d", timer.year, timer.month, timer.day,
...
{
	TRACE_(QS_USER, NULL, "[GPS] %d-%d-%d:%d-%d-%d, Lon=%x.%x%x%x, Lat=%x.%x%x%x, Speed=%x%x, view_S=%x, visib_S=%x",
			tGpsInfo.u16Year, tGpsInfo.u8Month, tGpsInfo.u8Day, tGpsInfo.u8Hour,
			tGpsInfo.u8Minute, tGpsInfo.u8Second,
			tGpsInfo.Longtitude[0], tGpsInfo.Longtitude[1], tGpsInfo.Longtitude[2], tGpsInfo.Longtitude[3],
			tGpsInfo.Latitude[0], tGpsInfo.Latitude[1], tGpsInfo.Latitude[2], tGpsInfo.Latitude[3],
			tGpsInfo.Speed[0], tGpsInfo.Speed[1], tGpsInfo.u8ValidSatellites, tGpsInfo.u8VisibleSatellites
			);
}
