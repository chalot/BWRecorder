...
{											    
	...
	while(VS_DREQ_STATE==0);     //等待DREQ为高
	//printf("mode sin:%x\n",VS_RD_Reg(SPI_MODE));
...
void AUDIO_Lowlevel_Init()
{
	...
	/*!< Enable GPIO clocks */
	RCC_AHB1PeriphClockCmd(AUDIO_SPI1_SCK_PIN | AUDIO_SPI1_MISO_PIN | AUDIO_SPI1_MOSI_PIN, ENABLE);

	/*!< SPI pins configuration *************************************************/

	GPIO_PinAFConfig(AUDIO_SPI1_SCK_PIN, AUDIO_SPI1_SCK_SOURCE, AUDIO_SPI1_SCK_AF);
	GPIO_PinAFConfig(AUDIO_SPI1_MISO_PIN, AUDIO_SPI1_MISO_SOURCE, AUDIO_SPI1_MISO_AF);
	GPIO_PinAFConfig(AUDIO_SPI1_MOSI_PIN, AUDIO_SPI1_MOSI_PIN, AUDIO_SPI1_MOSI_AF);
	...
	RCC_AHB1PeriphClockCmd(AUDIO_XDCS_PIN | AUDIO_DREQ_PIN | AUDIO_RST_PIN | AUDIO_XCS_PIN, ENABLE);
